...
(function () {
  ...
  async function handleButtonClick(e, clickedElement) {
    ...
    console.log("Found post slug:", postSlug);
  ...
  function renderLocationsList(locations) {
    ...
    if (locations.length === 0) {
      ...
      locations.forEach(function (loc, index) {
        ...
        if (loc.image) {
          html +=
            '<img class="location-item__image" src="' + loc.image + '" alt="' + escapeHtml(loc.name) + '">';
        }
  ...
  function createPopupContent(loc) {
    ...
    if (loc.image) {
      html +=
        '<img src="' + loc.image + '" alt="' + escapeHtml(loc.name) + '" style="border-radius: 8px; margin-bottom: 0.5rem;">';
    }
