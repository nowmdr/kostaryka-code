...
## 🔄 ТЕКУЩАЯ ЗАДАЧА: Условная загрузка скриптов (упрощенная)

**Статус**: 🟡 В процессе  
**Дата начала**: [добавь дату]  
**Зависит от**: 
- ✅ Этап 1 - Рефакторинг на Vanilla JS (завершен)
- ✅ Этап 2 - Lazy Loading + Единый Loader (завершен)

### Цель
Оптимизировать загрузку JavaScript и CSS - грузить только на страницах где функциональность карты используется.

### Проблема сейчас
Наш JS (~5KB) и loader CSS (~2KB) загружаются на **всех страницах сайта**, даже там где карта не нужна.

### Решение (упрощенное)
Загружать скрипты **только на 3 типах страниц**:
1. ✅ **Homepage** (главная страница)
2. ✅ **Single пост "oferta"** (`/oferta/nazwa-wyprawy/`)
3. ✅ **Страница с slug "oferta"** (`/oferta/`)
4. ❌ **Все остальные страницы** - не грузим

### Реализация

#### Простая проверка WordPress
```php
public function enqueue_scripts() {
    ...
    // 2. Single пост типа "oferta"
    if (is_singular('oferta')) {
        $should_load = true;
    }
    
    // 3. Страница со slug "oferta"
    if (is_page('oferta')) {
        $should_load = true;
    }
...
### Пояснение WordPress функций

| Функция | Что проверяет |
|---------|---------------|
| `is_front_page()` | Главная страница сайта (настроенная в Settings → Reading) |
| `is_home()` | Страница блога (если главная = блог, то true) |
| `is_singular('oferta')` | Single пост CPT "oferta" |
| `is_page('oferta')` | Страница со slug "oferta" |

### Debug режим (опционально)

Можно добавить для тестирования:
```php
public function enqueue_scripts() {
    // Debug: ?force_trip_map=1 в URL (только для админов)
    if (isset($_GET['force_trip_map']) && current_user_can('manage_options')) {
        $this->do_enqueue_scripts();
        return;
    }
...
Теперь админ может добавить `?force_trip_map=1` к любому URL чтобы принудительно загрузить скрипты.

### Тестирование

#### ✅ Страницы где ДОЛЖНЫ загружаться скрипты:
- [ ] `/` (главная)
- [ ] `/oferta/` (страница oferta)
- [ ] `/oferta/klejnoty-kostaryki/` (single пост)

#### ❌ Страницы где НЕ ДОЛЖНЫ загружаться:
- [ ] `/o-nas/` (обычная страница)
- [ ] `/kontakt/` (страница контактов)
- [ ] `/blog/` (блог)
- [ ] `/uslugi/` (другие страницы)

#### Как тестировать:
```
1. Открыть страницу
2. F12 → Network tab
3. Отфильтровать: trip-map
4. Обновить страницу (Ctrl+R)
5. Проверить:
   ✅ ДОЛЖНЫ: trip-map.js и loader.css загружены
   ❌ НЕ ДОЛЖНЫ: нет запросов на эти файлы
```
...
**Выигрыш: 7KB (100%) на страницах без карты**

### Чеклист выполнения

- [ ] Создана приватная функция `do_enqueue_scripts()`
- [ ] Переписана `enqueue_scripts()` с простыми условиями
- [ ] Добавлена проверка `is_front_page()` / `is_home()`
- [ ] Добавлена проверка `is_singular('oferta')`
- [ ] Добавлена проверка `is_page('oferta')`
- [ ] (Опционально) Добавлен debug режим `?force_trip_map=1`
- [ ] Протестировано на главной
- [ ] Протестировано на `/oferta/`
- [ ] Протестировано на single посте oferta
- [ ] Протестировано на `/kontakt/` (не должно грузиться)
- [ ] Проверен Network tab
- [ ] Обновлена документация

### Преимущества упрощенного подхода

1. **Простота** - всего 3 условия, легко понять
2. **Надежность** - WordPress функции всегда работают корректно
3. **Производительность** - минимум проверок, быстрое выполнение
4. **Поддержка** - легко изменить или расширить в будущем

### Итоговая оптимизация плагина

После завершения всех 3 этапов:

**Этап 1: Vanilla JS**
- Было: 36KB (5KB наш + 30KB jQuery)
- Стало: 5KB (без jQuery)
- Выигрыш: 31KB

**Этап 2: Lazy Loading**
- Было: 5KB + 52KB Leaflet = 57KB при загрузке страницы
- Стало: 5KB при загрузке, +52KB при открытии popup (1 раз)
- Выигрыш: 52KB при загрузке страницы

**Этап 3: Условная загрузка**
- Было: 5KB на всех страницах
- Стало: 0KB на страницах без карты
- Выигрыш: 5KB на ~80% страниц сайта

**ИТОГОВЫЙ РЕЗУЛЬТАТ:**
- Страница БЕЗ карты: 0KB (было 88KB)
- Страница С картой: 7KB при загрузке (было 88KB)
- Открытие popup: +52KB (только 1 раз за сессию)

**Общий выигрыш: ~88KB → ~7KB = 92% меньше!** 🎉
```
